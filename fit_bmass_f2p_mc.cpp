...
void fit_bmass_f2p_mc(){
  ...
  ch->Add("../sel_BsTof2pMuMu_MC_JPSI_*.root");
  TTree *tr = ch;
  ...
  TCut c8 = "Bcosalphabs2d>0.9992";
  TCut c9 = "Blxysig>8.5";
  TCut c10 = "Triggers==1";

  TCut cutTotal = c1 && c2 && c3 && c4 && c5 && c6 && c7 && c8 && c9 && c10;

  RooDataSet *redData = (RooDataSet*)data.reduce(cutTotal);
  ///RooDataSet *redData = (RooDataSet*)data.reduce(c1&&c2);
 

  RooRealVar  mean("mean","mean of the gaussians", 5.366, 5.2, 5.5);
  RooRealVar  sigma("sigma","sigma of the gaussian", 0.025, 0.005, .06);
  RooRealVar  sigma2("sigma2","sigma of the gaussian2", 0.065, 0.01, .1);
  ...
  //RooBreitWigner  sigG("sigG","signal component-1", Bmass, mean, sigma);
  RooGaussian  sigG1("sigG1","signal component-1", Bmass, mean, sigma);
  RooGaussian  sigG2("sigG2","signal component-2", Bmass, mean, sigma2);
  ...
  //RooExponential bkgC("bkgC","exponential PDF", Bmass, lambda);

  RooRealVar a0("a0", "constant", -20., 20.);
  RooRealVar a1("a1", "linear", -20., 20.);
  RooRealVar a2("a2", "quadratic", -20., 20.);
  //RooRealVar a0("a0", "constant", -.52);//, 10.);
  //RooRealVar a1("a1", "linear", -.1, 1.);
  //RooRealVar a2("a2", "quadratic", -.02);//, 2.);
  //RooChebychev bkgC("bkgC", "Background", Bmass, RooArgSet(a0, a1, a2));
  //RooChebychev bkgC("bkgC", "Background", Bmass, RooArgSet(a0, a1));
  //RooRealVar a0("a0", "constant", 10, 1.e6);
  //RooRealVar a1("a1", "linear", -1.e6, -1.);
  //RooRealVar a0("a0", "constant", 98000.,97000.,110000.);
  //RooRealVar a1("a1", "linear", -16667.,-17000., -16000.);
  RooPolynomial bkgC("bkgC", "Background", Bmass, RooArgSet(a0, a1));
  //RooPolynomial bkgC("bkgC", "Background", Bmass, RooArgSet(a0));
  ...
  RooPlot *xframe = Bmass.frame(Title("#mu#mu KK invariant mass"),Bins(30) ); //50
  ...
  paveText->AddText(Form("#sigma_{1} = %.4f #pm %.4f GeV/c^{2}" , sigma.getVal(), sigma.getError()));
  paveText->AddText(Form("#sigma_{2} = %.4f #pm %.4f GeV/c^{2}" , sigma2.getVal(), sigma2.getError()));
  //paveText->AddText(Form("width = %.4f #pm %.4f GeV/c^{2}" , width.getVal(), width.getError()));
  //paveText->AddText(Form("#lambda = %.4f #pm %.4f GeV/c^{2}" , lambda.getVal(), lambda.getError()));
  ...
  mark->DrawLatex(0.75,startY,"#scale[0.8]{4.21 fb^{-1} fb^{-1} (13 TeV)}");
